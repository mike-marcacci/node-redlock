...
function run(
  ...
  test(`${namespace} - acquires, extends, and releases a single lock`, async (t) => {
    ...
    t.is(
      Math.floor((await redisA.pttl("{redlock}a")) / 100),
      Math.floor(duration / 100),
      "The lock expiration was off by more than 100ms"
    );
    t.is(
      Math.floor((await redisB.pttl("{redlock}a")) / 100),
      Math.floor(duration / 100),
      "The lock expiration was off by more than 100ms"
    );
    t.is(
      Math.floor((await redisC.pttl("{redlock}a")) / 100),
      Math.floor(duration / 100),
      "The lock expiration was off by more than 100ms"
    );
  ...
  test(`${namespace} - succeeds when a minority of clients fail`, async (t) => {
    ...
    t.is(
      Math.floor((await redisA.pttl("{redlock}b")) / 100),
      Math.floor(duration / 100),
      "The lock expiration was off by more than 100ms"
    );
    t.is(
      Math.floor((await redisB.pttl("{redlock}b")) / 100),
      Math.floor(duration / 100),
      "The lock expiration was off by more than 100ms"
    );
    t.is(
